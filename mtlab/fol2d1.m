...
for i = 2:Is,
    ...
    if counter==1,
        [ends(1,2) ends(1,1) flag(1)]= ...
            growth2d(M,N,ends(1,2),ends(1,1),u, v, src(1,2), src(1,1), 1.4E-3, 2E-6, flag(1));
        [ends(2,2) ends(2,1) flag(2)]= ...
            growth2d(M,N,ends(2,2),ends(2,1),u, v, src(2,2), src(2,1), 1.4E-3, 2E-6, flag(2));
        [ends(3,2) ends(3,1) flag(3)]= ...
            growth2d(M,N,ends(3,2),ends(3,1),u, v, src(3,2), src(3,1), 1.4E-3, 2E-6, flag(3));  
        counter=100;
        ends
    end
    ...
    %generation part
    U(:,i)=step (nabla, u , w, dt, dx, Src, gu, I, O, 0.002,0.3,0.05);
    W(:,i)=step (nabla, w , u, dt, dx, Ends, gw, I, O, 0.003,0.3,0.05);
    V(:,i)=step (nabla, v , y, dt, dx, Ends, gv, I, O, 0.0005,0.4,0.05);
    Y(:,i)=step (nabla, y , v, dt, dx, Src, gy, I, O, 0.002,0.4,0.05);
    %diffusion rxn part
    u=U(:,i);
    w=W(:,i);
    v=V(:,i);
